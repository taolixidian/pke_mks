...
# 加密算法，输入安全参数param，G1,G2的生成元g1,g2，公钥pk，关键字集合w，返回[c1, ci]
...
# 陷门生成算法，输入安全参数param，G1,G2的生成元g1,g2，私钥sk，待搜索关键字集合w，返回[t1, t2, t3, ti]
def Trapdoor(g1, sk, pk, w, param):
    group = PairingGroup(param)
    r = group.random(ZR)
...
if __name__ == "__main__":
    ...
    print("time_enc(s):", round(time_enc * 1000, 3))

    s3 = time.time()
    td = Trapdoor(g1, sk, pk, w, param)
    time_trap = time.time() - s3
    print("time_trapdoor(s):", round(time_trap * 10000, 3))

    s4 = time.time()
    res = Test(ct, td)
    time_test = time.time() - s4
    print("time_test(s):", round(time_test * 1000, 3))
